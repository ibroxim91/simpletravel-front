...
export default function Discussion() {
    const [activeCategory, setActiveCategory] = React.useState<string>('О сервисе')
    return (
        <div className='custom-container'>
            ...
            <h1 className='font-bold text-[#031753] text-[32px] my-5'>Ответы на вопросы</h1>
            ...
            <div className='grid grid-cols-[20%_80%] gap-6'>
                <ul className='p-[20px] bg-[#ffffff] rounded-[16px] h-max'>
                    {
                        Object.keys(questions_answers).map((item, index) => (
                            <li key={index} onClick={() => setActiveCategory(item)} className={`cursor-pointer hover:text-[#031753]  py-[10px] px-[20px] ${activeCategory === item ? 'bg-[#EDEEF1] rounded-[10px]' : ''}`}>{item}</li>
                        ))
                    }
                </ul>
                ...
                <div className='w-full'>
                    {
                        questions_answers[activeCategory as keyof typeof questions_answers].map((item, index) => (
