...
const ForgetTwoSteps = ({ setStep }: Props) => {
  ...
  const { phone, email, setToken } = useLoginPhoneStore();
  ...
  const { mutate: phoneMutate, isPending } = useMutation({
    mutationFn: ({ phone, code }: { phone: string; code: string }) => {
      return Auth_Api.confirmResetPassPhone({ phone, code });
    },
    onSuccess(data) {
      setStep(3);
  ...
  const { mutate: emailMutate, isPending: emailPending } = useMutation({
    mutationFn: ({ email, code }: { email: string; code: string }) => {
      return Auth_Api.resetPassConfirmEmail({ email, code });
    },
    onSuccess(data) {
      setStep(3);
      setToken(data.data.data.token);
